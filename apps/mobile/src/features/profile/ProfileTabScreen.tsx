  ...
  return (
    <View style={[styles.screen, { backgroundColor: theme.canvas }]}>
      <SafeAreaView edges={['top', 'left', 'right']} style={styles.safeArea}>
        ...
        <View accessibilityLabel="Portfolio views" accessibilityRole="tablist" style={styles.tabs}>
...
function PortfolioAccountControl({
  ...
  return (
    <>
      <AppAccountButton onPress={accountReady ? () => setMode('account') : onOpenAuth} />
      <PortfolioSheet
        address={address}
    ...
    <>
      <View style={[styles.guestHero, { backgroundColor: theme.surface }]}>
        <View style={[styles.privateMark, { backgroundColor: theme.proofWash }]}>
          <LockIcon color={theme.proof} size={21} />
        </View>
      ...
      {loading ? (
        ...
        <View style={[styles.register, { borderTopColor: theme.outline }]}>
      ...
      <Text style={[styles.localNote, { color: theme.muted }]}>Saved companies are device-local. They are not holdings and are not synced to an account.</Text>

      <View accessibilityLabel="Portfolio information" style={[styles.utilityList, { borderTopColor: theme.outline }]}>
...
function SavedCompanyRow({ company, onPress }: { company: CompanySummary; onPress: () => void }) {
  ...
  return (
    <Pressable
      ...
      accessibilityRole="button"
      onPress={onPress}
      style={({ pressed }) => [styles.companyRow, { borderBottomColor: theme.outline }, pressed && styles.rowPressed]}>
      ...
      <View style={styles.companyValue}>
        ...
        <Text style={[styles.companyChange, { color: company.changePercent === null ? theme.muted : positive ? theme.proof : theme.caution }]}>
      ...
      <View style={styles.modalLayer}>
        ...
        <SafeAreaView edges={['bottom', 'left', 'right']} style={[styles.sheet, { backgroundColor: theme.surface }]}>
          ...
          <View style={styles.sheetHeader}>
            ...
            <Pressable accessibilityLabel="Close" accessibilityRole="button" onPress={onClose} style={({ pressed }) => [styles.closeButton, { backgroundColor: theme.canvas }, pressed && styles.pressed]}>
          ...
          {mode === 'account' ? (
            <>
              ...
              <View style={[styles.walletIdentity, { backgroundColor: theme.canvas }]}>
              ...
              <Text selectable style={[styles.address, { backgroundColor: theme.canvas, color: theme.muted }]}>{address ?? 'Wallet address is still loading'}</Text>
          ...
          {mode === 'receive' ? (
            <>
              ...
              <Text selectable style={[styles.receiveAddress, { backgroundColor: theme.canvas, color: theme.ink }]}>{address ?? 'Wallet address is still loading'}</Text>
          ...
          {mode === 'portfolio-info' ? (
            <>
              ...
              <View style={[styles.fact, { backgroundColor: theme.canvas }]}>
          ...
          {mode === 'about' ? (
            <>
              ...
              <View style={[styles.fact, { backgroundColor: theme.canvas }]}><Text style={[styles.factText, { color: theme.ink }]}>Market values are informational until you review an executable trade quote.</Text></View>
            </>
          ) : null}
...
function UtilityRow({ label, onPress }: { label: string; onPress: () => void }) {
  const theme = useTheme();
  return (
    <Pressable accessibilityRole="button" onPress={onPress} style={({ pressed }) => [styles.utilityRow, { borderBottomColor: theme.outline }, pressed && styles.pressed]}>
...
const styles = StyleSheet.create({
  screen: { flex: 1 },
  safeArea: { flex: 1 },
  header: { alignItems: 'center', borderBottomWidth: StyleSheet.hairlineWidth, flexDirection: 'row', justifyContent: 'space-between', minHeight: 64, paddingHorizontal: 18, paddingVertical: 8 },
  brand: { alignItems: 'center', flexDirection: 'row', gap: 9, minHeight: 44 },
  titleBar: { justifyContent: 'center', minHeight: 57, paddingHorizontal: 18, paddingVertical: 5 },
  titleCopy: { minWidth: 0 },
  title: { fontFamily: Fonts.sans, fontSize: 26, fontWeight: '800', letterSpacing: -1.2, lineHeight: 29 },
  subtitle: { fontFamily: Fonts.sans, fontSize: 11, marginTop: 4 },
  tabs: { flexDirection: 'row', gap: 26, minHeight: 44, paddingHorizontal: 18 },
  tab: { justifyContent: 'flex-end', minHeight: 44 },
  tabLabelRow: { alignItems: 'center', flexDirection: 'row', gap: 4, justifyContent: 'center', minHeight: 40 },
  tabLabel: { fontFamily: Fonts.sans, fontSize: 13, fontWeight: '700' },
  tabRule: { borderRadius: 2, height: 3, width: '100%' },
  scrollContent: { flexGrow: 1, paddingBottom: Spacing.five, paddingHorizontal: 18 },
  guestHero: { alignItems: 'center', borderRadius: 18, flexDirection: 'row', gap: 12, marginTop: 18, padding: 16 },
  privateMark: { alignItems: 'center', borderRadius: 14, height: 42, justifyContent: 'center', width: 42 },
  guestHeroCopy: { flex: 1, minWidth: 0 },
  guestHeroTitle: { fontFamily: Fonts.sans, fontSize: 16, fontWeight: '700', letterSpacing: -0.4, lineHeight: 20 },
  guestHeroBody: { fontFamily: Fonts.sans, fontSize: 12, lineHeight: 17, marginTop: 4 },
  heroButton: { alignItems: 'center', borderRadius: 13, flexDirection: 'row', gap: 7, justifyContent: 'center', minHeight: 44, paddingHorizontal: 13 },
  heroButtonText: { fontFamily: Fonts.sans, fontSize: 13, fontWeight: '800' },
  heroButtonArrow: { fontFamily: Fonts.sans, fontSize: 16 },
  sectionHeading: { alignItems: 'center', flexDirection: 'row', justifyContent: 'space-between', marginBottom: 9, minHeight: 32, paddingTop: 24 },
  sectionTitle: { fontFamily: Fonts.sans, fontSize: 17, fontWeight: '700', letterSpacing: -0.4 },
  sectionLabel: { fontFamily: Fonts.sans, fontSize: 12, fontWeight: '600' },
  ...
  companyLogo: { alignItems: 'center', borderRadius: 13, height: 44, justifyContent: 'center', overflow: 'hidden', width: 44 },
  companyLogoImage: { height: 44, width: 44 },
  companyInitials: { fontFamily: Fonts.sans, fontSize: 12, fontWeight: '800' },
  companyCopy: { flex: 1, minWidth: 0 },
  companyName: { fontFamily: Fonts.sans, fontSize: 14, fontWeight: '700' },
  companyMeta: { fontFamily: Fonts.sans, fontSize: 11, marginTop: 4 },
  companyValue: { alignItems: 'flex-end', maxWidth: 124 },
  companyPrice: { fontFamily: Fonts.mono, fontSize: 12, fontWeight: '600' },
  companyChange: { fontFamily: Fonts.mono, fontSize: 10, marginTop: 5 },
  ...
  emptySavedTitle: { fontFamily: Fonts.sans, fontSize: 14, fontWeight: '700' },
  ...
  utilityLabel: { fontFamily: Fonts.sans, fontSize: 13, fontWeight: '600' },
  ...
  sheetAction: { alignItems: 'center', borderRadius: 14, flex: 1, justifyContent: 'center', minHeight: 50, paddingHorizontal: 14 },
  sheetActionText: { fontFamily: Fonts.sans, fontSize: 13, fontWeight: '800', textAlign: 'center' },
