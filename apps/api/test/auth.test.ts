...
test('config rejects unsafe TTL, network, and production CORS invariants', () => {
  assert.equal(readConfig({ NODE_ENV: 'test', PHOENIX_REFERRAL_CODE: '' }).PHOENIX_REFERRAL_CODE, undefined);
  assert.equal(readConfig({ NODE_ENV: 'test', PHOENIX_REFERRAL_CODE: ' WARREN ' }).PHOENIX_REFERRAL_CODE, 'WARREN');
