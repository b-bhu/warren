...
const configSchema = z.object({
  ...
  DATABASE_URL: z.string().default('file:./stocklana.db'), AUTH_DOMAIN: z.string().default('localhost'), AUTH_URI: z.string().url().default('http://localhost:3000'),
...
export const readConfig = (env: NodeJS.ProcessEnv = process.env): Config => {
  ...
  if (config.NODE_ENV === 'production') {
    ...
    if (config.DATABASE_URL === 'file:./stocklana.db') throw new Error('DATABASE_URL must use a mounted production volume');
