...
| Scope | Expo iOS/Android OAuth return, MCP credential discovery, off-chain EVM/Solana signing, and Stocklana server verification |
...
This is a provider feasibility document, not an authorization to create an embedded wallet, store a seed phrase, move funds, or make provider authorization equivalent to a Stocklana session. The PRD remains authoritative: Stocklana alone issues the single-use challenge and accepts ownership only after its own server verifies the exact signed bytes.
...
3. Open `/oauth/authorize` in the system browser with `response_type=code`, `client_id`, the registered `redirect_uri`, `code_challenge`, `code_challenge_method=S256`, `scope=mcp` (plus `offline_access` only if refresh is needed), `resource=https://api.paybox.sh/mcp`, and Stocklana's random attempt-bound `state`.
...
**Inference — plausible, not verified:** A production `https://auth.stocklana.example/paybox/callback` can simultaneously be (1) the exact, registered Paybox redirect URI, and (2) an iOS Universal Link / Android App Link. Paybox's exact URI rule is compatible with that pattern because OAuth appends `code` and `state` after the registered path; Stocklana must register the exact scheme, host, path, trailing-slash form, and any static query component it uses. It must not generate per-attempt redirect URIs or substitute an Expo custom scheme.

Required conditions:

- Stocklana controls a stable HTTPS domain and serves the exact route without an intermediate URL rewrite that loses OAuth parameters.
...
- The installed-app case returns directly to the app. The uninstalled, disassociated, or browser-fallback case serves a safe Stocklana web page that does not expose the code, cannot create a session merely by loading, and gives an install/retry path. It must not redirect the code to a custom scheme.
...
In both designs, provider access is a separate authorization layer. Store provider credential ID, chain family, returned address, provider request ID, and Stocklana attempt ID as data; never treat an OAuth grant, `list_credentials` address, or Paybox `success` alone as a Stocklana login.
...
3. A Stocklana-controlled callback domain, DNS/TLS, iOS associated-domain entitlement + association file, Android intent-filter + asset-links file, actual iOS team/bundle ID, Android application ID, and release signing certificate SHA-256 fingerprint.
...
5. Stocklana backend endpoints for one-time challenge issue, exact-byte EVM/Solana verification, profile conflict handling, and Stocklana session issuance. Challenge TTL starts at 15 minutes for this test so it exceeds Paybox's documented ~10-minute approval window, then is reduced/approved based on measurement.
...
Use this contract to build Stocklana's provider-neutral state machine and server verifier before any Paybox account/client is used. It deliberately models only public artifacts and uses no real tokens, keys, identifiers, addresses, challenges, or signatures.

```json
{
  "oauthCallback": {
    "redirectUri": "https://auth.stocklana.test/paybox/callback",
...
4. **EVM ownership proof.** Grant only one EVM wallet credential. Call `list_credentials`, confirm a wallet / EVM family / non-null address, request exactly one `op: message` containing the Stocklana server's exact ERC-4361 text, handle approval, resume after backgrounding, and poll the same ID. Pass only if terminal output is a 65-byte `0x` signature that Stocklana verifies by ERC-191 recovery to the selected canonical address and chain ID; issue a Stocklana session only then.
5. **Solana ownership proof.** Repeat independently with only one Solana credential and one `op: solanaMessage`, with selected base58 address and exact SIWS-compatible UTF-8 text. Pass only if terminal output is exactly 64 bytes encoded as lowercase hex and Stocklana Ed25519 verification succeeds for the selected public key and cluster; issue a Stocklana session only then.
6. **Second-wallet link.** From an existing Stocklana session, repeat the other family. Pass only if it attaches to the authenticated profile, cannot create a duplicate profile, and a known conflict gets the PRD's privacy-preserving failure.
