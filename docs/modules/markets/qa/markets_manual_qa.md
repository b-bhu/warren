...
7. Search `anthropic`. Open the result and confirm the sheet shows every returned
   private-market capability without inventing a public ticker.
8. Tap any registry row. `Markets for [Company]` should open, focus the selected
   capability, show provider/product/value labels and a selectable instrument ID, and
   keep **Lend** and **Borrow** static under `Future`.
9. Close the sheet with its close button, the backdrop, and the platform back gesture.
   The selected tab, query, and list position should remain intact.
